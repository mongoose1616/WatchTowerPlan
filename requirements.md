...
| `query_harness` | `Current` at reusable-core root | Offer export-safe querying over pack surfaces, indexes, registries, routes, workflow metadata, and artifact families. | Keep reusable-core query exports focused on governed generic metadata while plan-domain query services remain owned by `watchtower_plan.query`. |
...
| `sync_harness` | `Current` | Coordinate authority-preserving synchronization across pack-owned and repo-owned surfaces. | The reusable-core sync harness now runs deterministic multi-target sync specs while plan-owned target catalogs remain under `watchtower_plan.sync`. |
...
| `watchtower_core.query` | `Current` | Export-safe reusable query boundary for governed surfaces, routes, commands, workflows, and artifact families, while fail-closing the remaining repo-local planning queries back to `watchtower_plan.query`. |
| `watchtower_core.sync` | `Current` | Export-safe reusable sync boundary for deterministic generic harness surfaces, while repo-local sync target implementations remain under `watchtower_plan.sync`. |
...
| `watchtower_plan` | `Current` | Approved plan-owned Python boundary for WatchTowerPlan-specific runtime, query, sync, validation, and authored-document semantics under `plan/python/src/watchtower_plan/`. |
...
1. Continue adding reusable query and sync helpers at the package root instead of letting plan-domain behavior leak back into reusable core.
2. Keep the Python mandate clean: reusable core remains under `watchtower_core`, while plan-domain runtime remains under the approved plan-owned Python boundary at `plan/python/src/watchtower_plan/`.
...
`WatchTowerPlan` already has enough machine authority, validation maturity, routing structure, and index coverage to justify treating it as the first serious internal `plan`-domain consumer of reusable core. The hard cutover to the first-class `plan/` workspace is now in place, and the remaining work is to keep the reusable core versus plan-domain Python split clean, continue rooting core-only documentation and workflows cleanly under `core/`, and preserve the mirrored foundations plus initiative-centric machine authority without reintroducing legacy planning sprawl.
