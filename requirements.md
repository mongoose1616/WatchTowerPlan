...
| `query_harness` | `Missing` at reusable-core root | Offer export-safe querying over pack surfaces, indexes, registries, routes, workflow metadata, and artifact families. | `watchtower_core.query` is currently only a guardrail root; real query logic still lives under `repo_ops`. |
...
| `watchtower_core.query` is only a guardrail namespace root. | A future pack model needs reusable query helpers that do not import repo-local planning query code. | `core/python/src/watchtower_core/query/README.md` | Introduce export-safe query services over declared surfaces, registries, routes, workflow metadata, and future artifact indexes. |
| `watchtower_core.sync` is only a guardrail namespace root. | Multi-pack and exported-core use cases need a real reusable sync boundary. | `core/python/src/watchtower_core/sync/README.md` | Introduce reusable authority-preserving sync helpers and drive `repo_ops` toward elimination rather than letting it remain the long-term orchestration boundary. |
