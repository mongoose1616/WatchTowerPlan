...
2. Compare docs or standards to current reality and governing family rules.
   ...
   - Confirm that each document uses the right document mode, required sections, and family template or standard, and flag stale assumptions, placeholder content, broken structure, or invented headings that ignore the governed template shape.
3. Review operationalization and companion surfaces.
   ...
   - When the correct doc shape is unclear, resolve it through `watchtower-core query templates` instead of inferring from one stale instance.
...
4. Refresh and reconcile the content.
   ...
   - Keep observed facts, inferred implications, and current local policy distinguishable when the document mixes source-backed explanation with repository rules.
