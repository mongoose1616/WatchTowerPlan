...
- Shared reusable workflow roles live under `core/workflows/roles/`; the active shared role set currently includes `workflow_steward.md` and `adversarial_reviewer.md`, with `adversarial_reviewer.md` now acting as a contradiction-oriented overlay for compatible review, validation, implementation, optimization, and remediation routes instead of only the dedicated repository-audit route.
