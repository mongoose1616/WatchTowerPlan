...
def build_guidance_front_matter(
    *,
    ...
    front_matter: dict[str, Any] = {
        ...
        "applies_to": [
            str(initiative_document["initiative_id"]),
            source_path,
            str(promotion_document["id"]),
...
def render_guidance_body(
    *,
    ...
    trace_id = str(initiative_document["trace_id"])
    evidence_refs = tuple(str(value) for value in initiative_document.get("evidence_ids", ()))
    promotion_id = str(promotion_document["id"])
    source_label = source_title or readable_source_artifact_kind(source_artifact_kind)
    source_summary_line = source_summary or initiative_summary
    common_provenance = (
        f"- Source initiative: `{initiative_id}` (`{trace_id}`)\n"
        f"- Source artifact: `{source_path}`\n"
        f"- Promotion record: `{promotion_id}`\n"
        f"- Evidence refs: {', '.join(f'`{value}`' for value in evidence_refs) if evidence_refs else '`none`'}"
    )
    if target_family == "reference":
        return (
            "# Subject Summary\n\n"
            f"{source_label} is promoted from `{source_path}` in `{initiative_title}`. "
            f"{source_summary_line}\n\n"
            "## Usage Guidance\n\n"
            f"- Use this reference when implementing or reviewing `{initiative_title}`-style guidance promotion flows.\n"
            f"- Treat `{promotion_id}` and `plan/.wt/registries/promotion_policy_registry.json` as the machine companions for this document.\n\n"
    ...
    if target_family == "decision_record":
        return (
            "# Context\n\n"
            f"`{initiative_title}` required a durable decision about how live initiative outputs become approved guidance. "
            f"{source_summary_line}\n\n"
            "## Decision\n\n"
            ...
            f"- Active via `{promotion_id}`.\n"
            f"{common_provenance}\n"
        )
    if target_family == "pattern":
        return (
            "# Scenario\n\n"
            f"Use this pattern when one initiative-local authored input needs to become durable shared guidance without turning `plan/docs/` into a second live workspace. "
            f"{source_summary_line}\n\n"
            ...
            "## Usage Notes\n\n"
            f"- This pattern was promoted from `{source_path}` in `{initiative_title}`.\n"
            f"{common_provenance}\n\n"
            "## Illustrative Example\n\n"
            f"- Promote `{source_path}` to `{target_path}` and keep the promotion record synchronized with the resulting guidance path.\n"
        )
    if target_family == "standard":
        return (
            "# Purpose\n\n"
            f"This standard captures the rule-bearing obligations for durable guidance promotion proven by `{initiative_title}`. "
            f"{source_summary_line}\n\n"
            "## Applicability\n\n"
    ...
    if target_family == "foundation":
        return (
            "# Purpose or Context\n\n"
            f"This foundation captures the durable boundary established by `{initiative_title}` for promotion-authority separation. "
            f"{source_summary_line}\n\n"
            "## Scope Boundary\n\n"
...
def guidance_title(
    *,
    ...
    return f"{initiative_title} {family_label}: {source_label}"
...
def guidance_summary(
    *,
    ...
    source_label = readable_source_artifact_kind(source_artifact_kind)
    return (
        f"Promoted {family_label} extracted from the {source_label} for {initiative_title}. "
        f"{compact}"
    )
