"""Private shared helpers for trace-scoped planning rendered snapshots."""
...
from watchtower_core.repo_ops.planning_rendered_source_assembly import (
    build_trace_planning_rendered_source_assembly,
)
...
class TracePlanningRenderedSnapshot:
    """Trace-scoped planning sources shared by initiative and catalog rendered surfaces."""
...
class TracePlanningCoordinationSnapshot:
    """Private coordination rendered surface derived from one trace snapshot."""
...
def build_trace_planning_rendered_snapshots(
    loader: ControlPlaneLoader,
) -> tuple[TracePlanningRenderedSnapshot, ...]:
    """Load all trace-scoped planning snapshots in traceability index order."""

    traceability_index = loader.load_traceability_index()
    sources = build_trace_planning_rendered_source_assembly(loader)
...
def build_trace_planning_coordination_snapshot(
    ...
    """Derive the shared coordination rendered surface for one trace snapshot."""
