...
class TaskLifecycleService:
    ...
    def update(self, params: TaskUpdateParams, *, write: bool) -> TaskMutationResult:
        ...
        if write and changed:
            ...
            if moved:
                self._repair_governed_task_path_references(
                    previous_doc_path=document.relative_path,
                    current_doc_path=relative_path,
                )
    ...
    def _repair_governed_task_path_references(
        self,
        *,
        ...
        self._rewrite_acceptance_contract_paths(
            previous_doc_path=previous_doc_path,
            current_doc_path=current_doc_path,
        )
        self._rewrite_validation_evidence_paths(
            previous_doc_path=previous_doc_path,
            current_doc_path=current_doc_path,
        )
    ...
    def _rewrite_acceptance_contract_paths(
        ...
        *,
        ...
        changed_any = False
        for relative_path, document in self._loader.iter_validated_documents_with_paths_under(
            ACCEPTANCE_CONTRACTS_DIRECTORY
        ):
            ...
            if changed:
                self._loader.schema_store.validate_instance(rewritten)
                self._loader.artifact_store.write_json_object(relative_path, rewritten)
                changed_any = True
                updated_documents.append((relative_path, rewritten))
            else:
                updated_documents.append((relative_path, document))
        if changed_any:
            self._loader.set_validated_directory_override(
                ACCEPTANCE_CONTRACTS_DIRECTORY,
