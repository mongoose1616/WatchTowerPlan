# `watchtower_core.plan_runtime.sync`

## Summary
Repository-local sync services for plan-pack derived indexes, rendered trackers, route and workflow metadata materialization, and GitHub task mirrors.
...
- Keep `plan_runtime/sync` focused on current pack-specific source selection, document joins, and write targets that still depend on the WatchTowerPlan layout.
