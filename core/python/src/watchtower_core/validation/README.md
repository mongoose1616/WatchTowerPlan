...
## Summary
Export-safe validation services, suite orchestration, aggregate baseline helpers, result models, and namespace guardrails that keep plan-domain semantic rule providers under `plan_runtime.validation`.
...
- `Non-Goals`: Direct export of repo-local document semantics implementations such as `DocumentSemanticsValidationService`; those stay under `watchtower_core.plan_runtime.validation` even when reusable-core orchestration invokes them.
