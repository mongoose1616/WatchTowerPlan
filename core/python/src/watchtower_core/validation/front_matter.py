"""Front-matter validation services backed by the governed validator registry."""
...
class FrontMatterValidationService:
    """Validate Markdown front matter through registry-backed validator selection."""

    def __init__(self, loader: ControlPlaneLoader) -> None:
        self._loader = loader

    def validate(self, path: str | Path, validator_id: str | None = None) -> ValidationResult:
        """Validate one Markdown document front-matter block."""
        ...
        issues = iter_schema_validation_issues(self._loader, front_matter, validator.schema_ids)
        return ValidationResult(
            validator_id=validator.validator_id,
            target_path=target_path,
            engine=validator.engine,
    ...
    def _resolve_validator(
        ...
        if not candidates:
            raise ValidationSelectionError(
                f"No active front-matter validator applies to {relative_target_path}."
            )
        if len(candidates) > 1:
            candidate_ids = ", ".join(sorted(candidate.validator_id for candidate in candidates))
            raise ValidationSelectionError(
                f"Multiple front-matter validators apply to {relative_target_path}: {candidate_ids}"
            )
    ...
    def _validate_registry_record(self, validator: ValidatorDefinition) -> ValidatorDefinition:
        ...
        if validator.artifact_kind != "documentation_front_matter":
            raise ValidationSelectionError(
                "Requested validator does not target documentation front matter: "
                f"{validator.validator_id}"
            )
        if validator.engine != "json_schema":
            raise ValidationExecutionError(
                f"Unsupported validator engine for front matter: {validator.engine}"
            )
