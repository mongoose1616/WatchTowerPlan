...
- When a shared-core test asserts pack-aware control-plane state, derive the expected validator, workflow, rendered-surface, authority, or tracking entry from the active `pack_settings.json` contract and merged registries instead of naming the current repository's concrete pack IDs or filenames.
...
- `./tools/install_git_hooks.sh` activates the tracked `.githooks/` directory by setting `core.hooksPath`. Run once per clone. The pre-push hook runs mypy and ruff (~0.2 s); full test suites are run manually via `verify.sh`.
