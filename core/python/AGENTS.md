...
- For agent execution, use the documented workspace entrypoint directly: prefer `uv run ...`; when a direct interpreter or tool binary is required after the workspace is synced, use `./.venv/bin/python` or `./.venv/bin/<tool>` from `core/python/`.
- Use `./tools/setup_dev_env.sh` from `core/python/` when the workspace has not been bootstrapped yet or when you need to repair the standard local environment before running WatchTower commands.
...
- Do not probe `python`, `python3`, `pip`, or other global aliases before the documented workspace entrypoints, and do not report their absence as a workflow issue when the workspace entrypoints exist.
...
- Do not start normal repository work by checking whether `python`, `python3`, or other global aliases exist on `PATH`.
