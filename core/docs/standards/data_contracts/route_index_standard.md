...
## Purpose
Provide a compact machine-readable rendered surface of the routing table so route-preview tooling can resolve task types and required workflow modules without reparsing the shared routing table and any pack-owned routing tables on every lookup.
...
- Every route entry must point only to workflow modules under the shared or owning-pack workflow module roots.
...
- Every `required_workflow_path` should exist under the shared or owning-pack workflow module roots.
