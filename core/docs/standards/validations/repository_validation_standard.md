...
- [documentation_semantics_standard.md](/core/docs/standards/documentation/documentation_semantics_standard.md): document-semantics validation should fail closed on shared Markdown guardrails such as repo-local link integrity.
...
- When a remediation task targets a non-`WatchTowerCore` repository but must land reusable canonical shared-core fixes in `WatchTowerCore/core`, validate the canonical slice with the shared-core baseline. Do not expand that proof path to `./tools/verify.sh all --pack plan` or other consumer-pack validation just because one consuming repository is `WatchTowerPlan`.
...
| Broad | Pre-closeout validation for one non-trivial change set | Active pack `sync all --write`, `watchtower-core validate all`, plus Python workspace checks when code changed |
...
- `Operational Surfaces`: `core/python/src/watchtower_core/validation/all.py`; `core/python/src/watchtower_core/validation/pack_contract.py`; `core/docs/commands/core_python/watchtower_core_validate_all.md`
