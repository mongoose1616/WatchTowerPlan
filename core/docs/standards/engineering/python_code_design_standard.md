...
## Use When
- Adding or refactoring Python modules under `core/python/src/watchtower_core/` or an owning pack root.
- Reviewing whether a new helper belongs in `control_plane/`, a reusable-core runtime package, an owning pack boundary, or `cli/`.
...
- Prefer extraction to a reusable-core package before growing a pack-owned package such as `watchtower_<pack>`. If logic is not specific to a pack-owned lifecycle, semantic rule set, or artifact layout, move it out of the pack-owned boundary.
...
- `watchtower_<pack>` packages under their owning pack roots own repository-local or pack-local orchestration that still depends on pack-owned lifecycle rules, rendered surfaces, or semantic validation.
...
- Do not bind shared-core integration tests to one concrete live pack root through direct filesystem references such as `REPO_ROOT / "plan"`. If one exact pack-root scenario still matters, keep it pack-owned or add an explicit gate so copied shared-core suites remain portable.
...
- `Operational Surfaces`: `core/python/src/watchtower_core/`; `core/python/src/watchtower_host/`; `core/python/tests/`; `core/python/AGENTS.md`; `core/python/README.md`; `core/docs/standards/engineering/python_workspace_standard.md`; `core/docs/standards/engineering/engineering_best_practices_standard.md`
...
- Reviewers should reject new generic behavior placed in pack-owned packages such as `watchtower_<pack>` when a reusable-core package boundary fits.
...
- When a helper is extracted from a pack-owned package such as `watchtower_<pack>`, that is usually a sign that this standard is working as intended. The goal is a narrow pack-owned domain boundary, not a second generic package root.
