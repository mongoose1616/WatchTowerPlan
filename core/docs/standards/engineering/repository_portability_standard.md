...
- Reviewers should reject engineering shared-core tests, docs, or workflow references that hard-code one donor pack's validator IDs, workflow IDs, rendered surfaces, or tracking filenames instead of deriving pack-aware expectations from the active pack contract.
- Reviewers should reject engineering shared-core tests that reach directly into one live donor pack root, such as `REPO_ROOT / "plan"`, when the same behavior can be proved through fixture packs, pack-owned tests, or an explicit conditional gate around that pack-owned scenario.
