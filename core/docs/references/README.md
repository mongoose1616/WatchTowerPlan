...
- Treat these files as supporting lookup artifacts. Enforceable local policy still belongs in `core/docs/standards/**` or `plan/docs/standards/**` and executable procedure still belongs in `core/workflows/**` or `plan/workflows/**`.
...
| Path | Description |
|---|---|
| `core/docs/references/README.md` | Describes the purpose of the external references directory and the fastest ways to find topic-specific references. |
| `core/docs/references/AGENTS.md` | Defines directory-specific rules for authored external reference documents. |
| `core/docs/references/agent_workflow_authoring_reference.md` | Working reference for efficient workflow-module authoring for agents and LLM-oriented tasks. |
| `core/docs/references/json_schema_2020_12_reference.md` | Working reference for the JSON Schema version used across governed machine-readable artifacts. |
| `core/docs/references/uv_reference.md` | Working reference for the Python workspace package and environment toolchain. |
