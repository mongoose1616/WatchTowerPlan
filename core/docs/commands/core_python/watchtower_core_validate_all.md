...
- The command must be launched from inside the target repository worktree, normally `core/python/`. When the current working directory is outside a valid repo, it exits non-zero instead of validating whichever package checkout provided the executable.
