...
```sh
cd core/python
uv run watchtower-core query standards --reference-path core/docs/references/github_collaboration_reference.md --format json
```
...
```sh
cd core/python
uv run watchtower-core query standards --operationalization-path core/docs/references/AGENTS.md --format json
```
