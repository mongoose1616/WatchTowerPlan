...
```sh
cd core/python
uv run watchtower-core validate document-semantics --path <path> [--validator-id <validator_id>] [--pack-settings-path <path>] [--format <human|json>]
```
...
```sh
cd core/python
uv run watchtower-core validate document-semantics --path core/docs/standards/documentation/workflow_md_standard.md --format json
```
