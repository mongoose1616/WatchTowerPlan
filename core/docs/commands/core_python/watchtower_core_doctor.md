...
| Source Surface | `core/python/src/watchtower_host/cli/doctor_family.py` |
...
- The command must be launched from inside the target repository worktree, normally `core/python/`. When the current working directory is outside a valid repo, it exits non-zero instead of falling back to whichever package checkout provided the executable.
