...
- The command reads the governed standards under `core/docs/standards/` and `plan/docs/standards/` and rebuilds the machine-readable standard index deterministically.
...
- Standards that cite raw external URLs without also citing a governed local reference doc under `core/docs/references/` are rejected during rebuild.
...
- `core/python/src/watchtower_core/plan_runtime/sync/standard_index.py`
