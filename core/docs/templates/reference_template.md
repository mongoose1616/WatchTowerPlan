...
> Make `## Quick Reference or Distilled Reference` the main value of the document, not a token preface to the upstream links.
...
> Delete any section that does not help the document stay focused on one bounded standard, topic, or area.
> Do not trim materially distinct rules, defaults, edge cases, or failure modes just to keep sibling sections the same size.
