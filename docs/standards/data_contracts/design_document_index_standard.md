...
- Covers placement, entry shape, update expectations, and the relationship between the index and the human-readable design tracker in `docs/design/`.
- Does not replace the design documents themselves or turn the index into design authority.

## Use When
- Adding a new feature design or implementation plan under `docs/design/`.
...
- [design_tracking.md](/home/j/WatchTowerPlan/docs/design/design_tracking.md)
...
- Keep design semantics in the documents under `docs/design/**`.
...
- Catalog only durable feature designs and implementation plans that live under `docs/design/features/` or `docs/design/implementation/`.
- Every index entry must point to an existing design document under `docs/design/`.
...
| `last_synced` | Required | Date string matching the design document’s `Last Synced` value. |
...
## Process or Workflow
1. Add or update the design document under `docs/design/`.
2. Update the human-readable design tracker in `docs/design/design_tracking.md` in the same change set.
...
## Examples
- A feature design like `docs/design/features/schema_resolution_and_index_search.md` should appear as a `feature_design` entry with its design title and summary.
- An implementation plan like `docs/design/implementation/control_plane_loaders_and_schema_store.md` should appear as an `implementation_plan` entry and name the feature designs it depends on in `source_paths`.
...
- Every `doc_path` should exist and point to a file under `docs/design/`.
