...
## Guidance
- Model the planning catalog as an index, not as a registry or tracker.
...
- Keep the planning catalog derived from current governed sources rather than hand-authoring it directly.
- Store published planning catalogs under `core/control_plane/indexes/planning/`.
...
- Update the companion artifact schema, examples, live planning catalog, loader support, query surface, and command docs in the same change set when the planning-catalog family changes structurally.
