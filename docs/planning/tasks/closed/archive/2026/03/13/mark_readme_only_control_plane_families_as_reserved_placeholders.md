...
summary: Clarify reserved maturity for README-only schema-index and registry-index
  families across family entrypoints and adjacent guidance.
type: task
status: active
task_status: done
...
## Summary
Clarify reserved maturity for README-only schema-index and registry-index families across family entrypoints and adjacent guidance.

## Scope
- Review control-plane index family entrypoints for README-only placeholder surfaces that currently read like active artifact families.
- Update parent and leaf READMEs so schema-index and registry-index directories are explicitly reserved until governed artifacts exist.
