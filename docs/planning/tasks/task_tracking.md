...
| `task.core_export_hardening_followup.sync_output_dir.001` | `ready` | `high` | `repository_maintainer` | `trace.core_export_hardening_followup` | `docs/planning/tasks/open/core_export_sync_output_snapshots.md` | Ensure sync output directories feed later dependent targets from the materialized snapshot instead of stale canonical derived artifacts. | `None` |
