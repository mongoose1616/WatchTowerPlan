...
- If the closeout-tracking entry review passes, implement one bounded Phase 4 closeout-tracking refresh-boundary slice, stop it at an explicit outcome review, and keep broader tracker-family convergence and later-phase work blocked.
...
- `req.structural_rewrite_program.019`: Before any closeout-tracking implementation begins, the trace must record an explicit closeout-tracking entry review outcome that reaffirms the approved shared closeout seam, preserves the explicit closeout result contract fields, names one private closeout-local refresh boundary for the remaining direct tracker outputs, and opens one bounded implementation task plus ready ledgers for the approved slice.
- `req.structural_rewrite_program.020`: If the closeout-tracking entry review passes, the approved Phase 4 closeout-tracking slice must stay fixed to one private closeout-local refresh boundary for `prd_tracking_output_path`, `decision_tracking_output_path`, and `design_tracking_output_path` after the approved shared closeout seam while preserving the five public planning-authority answers, the stable closeout result contract, the public tracker sync family names, and the current `TaskLifecycleService` and `PlanningScaffoldService` behavior.
- `req.structural_rewrite_program.021`: If the approved closeout-tracking refresh-boundary slice lands, the trace must publish the completed implementation task, its migration record, its validation-evidence artifact, and one explicit outcome-review task while preserving the five public planning-authority answers, the approved shared closeout seam, the stable closeout result contract, the public tracker sync family names, the current mutation callers, and the private planning-graph boundary.
- `req.structural_rewrite_program.022`: After the approved closeout-tracking refresh-boundary slice lands, the next controlling surface must be one explicit outcome-review task that keeps broader Phase 4, tracker-family, and later-phase rollout blocked until that review records a pass or block decision or one explicit program-closeout decision.
...
- `ac.structural_rewrite_program.021`: Before any closeout-tracking implementation begins, the trace records the explicit closeout-tracking entry review outcome, preserves the approved shared closeout seam and explicit closeout result contract boundary, and opens one bounded implementation task plus ready ledgers for the approved slice.
- `ac.structural_rewrite_program.022`: If the closeout-tracking entry review passes, the approved Phase 4 closeout-tracking slice plan and implementation task stay limited to one private closeout-local refresh boundary for `prd_tracking_output_path`, `decision_tracking_output_path`, and `design_tracking_output_path` while the five public planning-authority answers, stable closeout result-contract fields, public tracker sync family names, current mutation callers, and private planning-graph boundary remain unchanged.
- `ac.structural_rewrite_program.023`: If the approved closeout-tracking refresh-boundary slice lands, the trace publishes the completed implementation task, its migration and validation-evidence artifacts, and one explicit outcome-review task while preserving the five public planning-authority answers, the approved shared closeout seam, the stable closeout result contract, public tracker sync family names, current mutation callers, and the private planning-graph boundary.
- `ac.structural_rewrite_program.024`: After the bounded closeout-tracking refresh-boundary slice lands, one explicit outcome-review task becomes the next controlling surface and keeps broader Phase 4, tracker-family, and later-phase rollout blocked until that review records a pass or block decision or one explicit program-closeout decision.
