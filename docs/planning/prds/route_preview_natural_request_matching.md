...
## Problem Statement
The final review request still reproduces a live routing gap. When route preview is asked to score:

`review the regression findings and fix the valid issues with planning, tasks, validation, and commits`
...
- `req.route_preview_natural_request_matching.002`: The exact final review request used in this audit must select `Task Lifecycle Management` and `Commit Closeout` in addition to the already-matched review and validation routes.
...
- `ac.route_preview_natural_request_matching.002`: `watchtower-core route preview --request "review the regression findings and fix the valid issues with planning, tasks, validation, and commits" --format json` returns `Repository Review`, `Task Lifecycle Management`, `Code Validation`, and `Commit Closeout`, and does not regress into `Code Review`.
...
- [engineering_design_principles.md](/docs/foundations/engineering_design_principles.md): close the gap with one explicit, bounded seam instead of a larger routing redesign.
- [repository_standards_posture.md](/docs/foundations/repository_standards_posture.md): keep authored routing guidance, machine-readable lookup, docs, tests, and evidence aligned.
- [product_direction.md](/docs/foundations/product_direction.md): improve the shared-core workflow support layer without expanding route preview into autonomous product behavior.

## References
- March 2026 workflow and governance review summary for the reproduced route-preview gap.
- March 2026 remediation-program summary for the bounded route-preview follow-up.
