...
- `<validate_command>`: Choose the validation surface you want to run, currently `front-matter`, `artifact`, or `acceptance`.
...
- The current leaf commands are `all` for aggregate repo validation, `front-matter` for governed Markdown metadata, `artifact` for schema-backed JSON artifacts, and `acceptance` for semantic reconciliation across PRD acceptance, contracts, evidence, validators, and traceability.
