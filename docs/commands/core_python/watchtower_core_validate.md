...
- The current leaf commands are `front-matter` for governed Markdown metadata, `artifact` for schema-backed JSON artifacts, and `acceptance` for semantic reconciliation across PRD acceptance, contracts, evidence, validators, and traceability.
