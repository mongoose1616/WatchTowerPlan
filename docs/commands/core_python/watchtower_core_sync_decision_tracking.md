...
## Summary
This command rebuilds the human-readable active-first decision tracker from the governed decision index and the initiative-closeout state stored in traceability.
...
- The rebuilt tracker inlines active decisions, summarizes terminal trace history by initiative status, and points closed-trace lookup to `watchtower-core query initiatives`, `watchtower-core query decisions`, and `watchtower-core query planning`.
