...
## Summary
This command rebuilds the human-readable active-first design tracker from the governed design index and the initiative-closeout state stored in traceability.
...
- The rebuilt tracker inlines active feature designs and implementation plans, summarizes terminal trace history by initiative status, and points closed-trace lookup to `watchtower-core query initiatives`, `watchtower-core query designs`, and `watchtower-core query planning`.
