...
- You need a quick view of whether governed front matter, schema-backed artifacts, and acceptance reconciliation currently pass.
...
```sh
cd core/python
uv run watchtower-core validate all [--skip-front-matter] [--skip-artifacts] [--skip-acceptance] [--format <human|json>]
```
...
- The current validation families are governed front matter, schema-backed governed artifacts, and acceptance reconciliation across traceability surfaces.
- Acceptance reconciliation is included by default, so the command may fail when active traces do not yet have complete PRD, contract, or evidence coverage.
