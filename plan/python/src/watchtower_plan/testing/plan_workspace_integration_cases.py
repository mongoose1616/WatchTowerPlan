...
def test_plan_workspace_sync_writes_indexes_views_and_query_surfaces(
    ...
    plan_view = (repo_root / "plan/initiatives/workspace_alpha/plan.md").read_text(encoding="utf-8")
    ...
    assert coordination_index.recommended_surface_path == (
        "plan/initiatives/workspace_alpha/progress.md"
    )
...
def test_plan_workspace_sync_builds_promotion_and_guidance_indexes_and_validates_them(
    ...
    assert not any(entry.doc_path.startswith("plan/docs/commands/") for entry in guidance_entries)
...
def test_guidance_promotion_service_promotes_outputs_and_updates_indexes(
    ...
    assert {
        family for envelope in extraction_envelopes for family in envelope.knowledge_families
    } == {
        "decision_record",
        "pattern",
        "reference",
    ...
    assert {envelope.source_note_id for envelope in result.extraction_envelopes} == {
    ...
    promotion_record = _load_json(
        repo_root
        / (
            "plan/initiatives/workspace_guidance_promotion/.wt/promotions/"
...
def test_guidance_promotion_service_fans_out_foundation_mirrors(
    ...
    promotion_record_path = repo_root / (
        "plan/initiatives/workspace_foundation_promotion/.wt/promotions/"
        "guidance_promotion_record.bootstrap.json"
    )
    ...
    promotion_record["candidates"] = [
        {
            ...
            "provenance_expectation": (
                "Promotions must cite the source initiative id, trace id, and evidence bundle id."
            ),
    ...
    assert output.target_path == (
        "plan/docs/foundations/workspace_foundation_promotion_initiative_brief.md"
    )
...
def test_plan_workspace_sync_uses_latest_task_state_timestamp_for_indexes(
    ...
    summary_view = (repo_root / "plan/initiatives/workspace_delta/summary.md").read_text(
        encoding="utf-8"
    )
...
def test_plan_workspace_sync_supports_closing_initiatives_with_no_open_tasks(
    ...
    assert initiative_entry["next_action"] == (
