...
def test_bootstrap_packwide_initiative_writes_full_package_and_stages_review(
    ...
    assert (initiative_root / ".wt" / "evidence" / "validation_bundle.bootstrap.json").exists()
    assert (initiative_root / ".wt" / "closeout" / "closeout_recap.bootstrap.json").exists()
    assert (
...
def test_bootstrap_packwide_initiative_blocks_readiness_when_blocking_deferred_items_exist(
    ...
    assert any("Blocking deferred item" in message for message in readiness.issue_messages)
...
def test_authored_input_drift_requires_confirmation_before_review_is_restored(
    ...
    state = _initiative_state(repo_root)
    assert state["lifecycle_stage"] == "capture_incomplete"
    assert state["discrepancy_ids"] == [f"discrepancy.{INITIATIVE_SLUG}.initiative_brief_drift"]
    discrepancy = _load_json(
        _initiative_root(repo_root) / ".wt" / "discrepancies" / "initiative_brief_drift.json"
    )
...
def test_machine_root_human_surface_policy_blocks_stray_readme(
    ...
    assert any(
        "Forbidden human surface is present" in message for message in readiness.issue_messages
    )
            ...
            json.dumps(
                {
                    ...
                    'payload': {
                        'task_id': (
                            'task.watchtower_runtime_bootstrap.seed_watchtower_project_initiative'
                        )
                    },
                },
                indent=2,
