...
class TraceAccumulator:
    ...
    has_live_initiative_state: bool = False
    ...
    source_surface_paths: set[str] = field(default_factory=set)
    ...
    def build_document(self) -> dict[str, object]:
        ...
        self._set_list_field(document, "source_surface_paths", self.source_surface_paths)
    ...
    def should_publish(self) -> bool:
        if self.has_live_initiative_state:
            return True
        return self.initiative_status != "active" or bool(self.task_ids)

    def preserve_existing_state(self, entry: dict[str, Any]) -> None:
        current_status = self.initiative_status
        initiative_status = entry.get("initiative_status")
        if (
            current_status == "active"
            and isinstance(initiative_status, str)
            and initiative_status
        ):
            self.initiative_status = initiative_status
        existing_updated_at = entry.get("updated_at")
        closed_at = entry.get("closed_at")
        if self.closed_at is None and isinstance(closed_at, str) and closed_at:
            self.closed_at = closed_at
        closure_reason = entry.get("closure_reason")
        if self.closure_reason is None and isinstance(closure_reason, str) and closure_reason:
            self.closure_reason = closure_reason
        superseded_by_trace_id = entry.get("superseded_by_trace_id")
        if (
            self.superseded_by_trace_id is None
            and isinstance(superseded_by_trace_id, str)
            and superseded_by_trace_id
        ):
