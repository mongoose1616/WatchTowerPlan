...
class PlanWorkspaceService:
    ...
    def _build_documents(
        ...
        existing_workspace_updated_at = _latest_timestamp(
            (
...
def _next_action(
    ...
    if readiness.ready_for_execution:
        ...
        if "in_progress" in active_task_statuses:
            return "Advance the current in-progress task set and keep initiative-local task state current."
        ...
        if "planned" in active_task_statuses:
            return "Promote the next planned task to ready before opening follow-up work."
...
def _existing_document_updated_at(path: Path) -> str:
    if not path.exists():
        return ""
    try:
    ...
    if not isinstance(document, dict):
        return ""
    return _document_updated_at(document)


def _latest_timestamp(values: object, *, fallback: str = "") -> str:
