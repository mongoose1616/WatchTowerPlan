...
class DocumentSemanticsValidationService:
    ...
    def _validate_document(
        ...
        *,
        ...
        if validator_id == INITIATIVE_HANDOFF_DOCUMENT_SEMANTICS_VALIDATOR_ID:
            self._validate_initiative_handoff_document(
                relative_target_path,
                resolved_path,
            )
            return
    ...
    def _validate_initiative_handoff_document(
        ...
        if filename in INITIATIVE_TEMPLATE_EXACT_SEMANTICS_FILENAMES:
            ...
            required_section_groups = INITIATIVE_GENERIC_REQUIRED_SECTION_GROUPS.get(
                filename
            )
    ...
    def _validate_initiative_cold_start_read_order(
        ...
        missing_docs = [
            name for name in required_docs if name not in read_order_section
        ]
        if missing_docs:
        ...
        canonical_doc_positions = [
            read_order_section.index(name) for name in required_docs[:-1]
        ]
    ...
    def _validate_markdown_title_and_required_section_groups(
        ...
        *,
        ...
        for section_group in required_section_groups:
            matched_title = next(
                (title for title in section_group if title in sections),
                None,
            )
