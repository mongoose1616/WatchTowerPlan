...
|---|---|---|---|
| `phase.0` | acceptance-contract validation, validation-evidence validation, traceability-index validation when machine companions changed, acceptance reconciliation, `plan query trace`, `plan query readiness`, `plan query coordination` | confirm one real ready task, no hidden baseline contradiction, and that the exact Phase 0 proof sequence runs without inferred repair steps | current acceptance contract, durable evidence record, traceability join proof when machine companions changed, coordination/readiness query output |
