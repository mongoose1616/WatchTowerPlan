...
|---|---|---|---|
| `phase.0` | initiative-local validation-bundle validation, traceability-index validation when machine companions changed, `plan query trace`, `plan query readiness`, `plan query coordination` | confirm one real ready task, no hidden baseline contradiction, and that the exact Phase 0 proof sequence runs without inferred repair steps | current initiative-local validation bundle, traceability join proof when machine companions changed, coordination/readiness query output |
