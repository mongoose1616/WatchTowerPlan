...
title: "Planning Retention and Cleanup Standard"
summary: "This standard defines the promote-then-delete retention model for closed traced planning artifacts so active policy lives in current canonical surfaces instead of accumulating indefinitely in historical trace packages."
type: "standard"
status: "active"
tags:
  - "standard"
...
# Planning Retention and Cleanup Standard

## Summary
This standard defines the promote-then-delete retention model for closed traced planning artifacts so active policy lives in current canonical surfaces instead of accumulating indefinitely in historical trace packages.
...
- Define one safe trace-level cleanup boundary without silently breaking traceability or leaving partial cleanup behind.
...
- Implementing or reviewing a repo-local cleanup workflow for traced planning artifacts.

## Related Standards and Sources
- [traceability_standard.md](/plan/docs/standards/governance/traceability_standard.md): cleanup must preserve explicit surviving authority and must not create silent trace-link drift.
...
- [initiative_closeout_standard.md](/plan/docs/standards/governance/initiative_closeout_standard.md): only terminal traces are eligible for cleanup consideration.
- [task_tracking_standard.md](/plan/docs/standards/governance/task_tracking_standard.md): initiative-local task state remains governed in place until explicit cleanup removes the full trace package.
...
- Clean up by `trace_id`, not by individual file family.
- An explicit cleanup must remove the whole trace package in one governed change set, including:
...
| Cleaned trace | Trace-local package removed; surviving authority remains only in promoted canonical artifacts and rebuilt live indexes. |

### Surviving authority after cleanup
...
| Current machine-readable planning state | Rebuilt initiative, task, traceability, coordination, and related live indexes after cleanup |
...
4. Record the cleanup decision and the surviving canonical authority surfaces in the governing change set, release notes, or other retained current context when needed.
5. Remove the whole trace package in one governed change set.
6. Rebuild the affected planning trackers and machine-readable indexes.
7. Validate that the removed trace no longer appears in retained planning surfaces.

## Operationalization
- `Modes`: `documentation`; `artifact`; `workflow`
- `Operational Surfaces`: `plan/initiatives/`; `plan/projects/`; `plan/python/src/watchtower_plan/`; `plan/python/src/watchtower_plan/cli/`

## Validation
- A purged trace should have no surviving open tasks.
- Surviving canonical surfaces should not reference removed trace-local paths after cleanup.
- Rebuilt planning trackers and indexes should no longer surface the removed trace as a retained planning package.
- Reviewers should reject partial or ad hoc file deletion that bypasses the trace-level cleanup boundary.

## Change Control
- Update this standard when the cleanup eligibility rules or surviving authority model change materially.
- Update the closeout guidance, retention policy registry, planning trackers, and affected indexes in the same change set when this standard changes structurally.
