...
- [repository_maintenance_loop_standard.md](/core/docs/standards/operations/repository_maintenance_loop_standard.md): rewrite slices still need the normal inspect, refresh, rebuild, validate, and record loop.
...
- Treat retained rewrite task documents as supporting history while the trace remains retained. Do not make a specific historical task path the enduring operational authority once the checkpoint document, acceptance contract, migration record, and validation-evidence artifact already carry the lasting control package.
...
- Reviewers should reject rewrite guidance that makes a specific retained historical task document the only enduring operational authority for a closed checkpoint.
...
## References
- [rewrite_surface_classification_standard.md](/plan/docs/standards/governance/rewrite_surface_classification_standard.md)
