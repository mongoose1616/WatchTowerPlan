...
- [planning_index_family_standard.md](/plan/docs/standards/data_contracts/planning_index_family_standard.md): defines the shared derived-index baseline and discoverability contract this family-specific standard narrows.
- [initiative_index_standard.md](/plan/docs/standards/data_contracts/initiative_index_standard.md): defines the initiative-family rendered surface that this coordination layer reads from.
- [task_index_standard.md](/plan/docs/standards/data_contracts/task_index_standard.md): defines the task authority layer behind actionable work and blockers.
- [traceability_index_standard.md](/plan/docs/standards/data_contracts/traceability_index_standard.md): defines the durable trace join that initiative state still mirrors.
- [coordination_tracking_standard.md](/plan/docs/standards/governance/coordination_tracking_standard.md): defines the derived human companion tracker derived from this index.
- [initiative_tracking_standard.md](/plan/docs/standards/governance/initiative_tracking_standard.md): defines the human initiative coordination layer that remains available beneath this machine-first view.
- [schema_catalog_standard.md](/core/docs/standards/data_contracts/schema_catalog_standard.md): defines the schema-catalog update expectations for this artifact family.
- [timestamp_standard.md](/core/docs/standards/metadata/timestamp_standard.md): defines the timestamp format used by coordination records.
- [coordination_tracking_standard.md](/plan/docs/standards/governance/coordination_tracking_standard.md): defines the derived human companion tracker that should remain aligned with this machine surface.

## Guidance
- Apply the shared planning-index-family baseline in [planning_index_family_standard.md](/plan/docs/standards/data_contracts/planning_index_family_standard.md).
...
- Carry recent closeout summaries as the current-navigation context rather than reproducing the full closed-history corpus.
...
- Keep the coordination index focused on current-state routing instead of recreating family-authority detail.
...
| `summary` | Required | Clear explanation of the current coordination state. |
...
| `actionable_tasks` | Required | Focused actionable-task summaries when present. Use an empty array when none are actionable. |
| `recent_closed_initiatives` | Required | Recent closeout context. Use an empty array when none are available. |
...
| `entries` | Keep initiative-shaped summaries derived from the initiative index and limited to active current-state initiatives. |
| `actionable_tasks` | Include trace context so a caller can jump directly to the right initiative and task surface. |
| `recent_closed_initiatives` | Publish only the recent context needed for navigation and review handoff. Do not duplicate full closed-history initiative entries here. |
...
- The coordination index should remain focused enough that it improves machine navigation instead of recreating family-authority detail.
...
- [task_index_standard.md](/plan/docs/standards/data_contracts/task_index_standard.md)
