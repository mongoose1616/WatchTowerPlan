# `watchtower-core plan closeout initiative`
...
| Field | Value |
|---|---|
| Invocation | `watchtower-core plan closeout initiative` |
| Kind | `subcommand` |
...
```sh
cd core/python
uv run watchtower-core plan closeout initiative --initiative-slug <initiative_slug> [--project-slug <project_slug>] --initiative-status <completed|superseded|cancelled> --closure-reason <reason> [--superseded-by-trace-id <trace_id>] [--closed-at <timestamp>] [--write] [--format <human|json>]
```
...
```sh
cd core/python
uv run watchtower-core plan closeout initiative --project-slug watchtower --initiative-slug watchtower_work_item_notes --initiative-status completed --closure-reason "Implemented and validated work-item notes" --format json
```
...
| `watchtower-core plan closeout retained-initiative` | Use this instead when the target is a retained trace record rather than a live initiative package under `plan/**`. |
...
| `watchtower-core plan sync all` | Rebuilds the broader derived surface set that includes the indexes and rendered views this command updates directly. |
