...
- Active routing, workflow metadata, standards, command docs, schemas, README surfaces, helper code, and tests still expose retired planning-model terminology and stale phase names even though live initiative packages already use `initiative_brief.md`, `design_record.md`, `implementation_slice.md`, and optional `decision_notes.md`.
...
- `task.plan_requirements_decisions_adherence_remediation.replace_planning_semantics`: Replace retired planning-document semantics with initiative-package authored-input semantics.
